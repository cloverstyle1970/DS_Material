...
export default function VendorsClient({ initial }: Props) {
  ...
  const TYPE_CLS_DARK: Record<VendorType, string> = {
    ...
    매출: "bg-green-900/60 text-green-300",
    공통: "bg-purple-900/60 text-purple-300",
  };
  const activeCls = isDark ? TYPE_CLS_DARK : TYPE_CLS;
                  ...
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <div className="flex gap-1">
                      <button type="button" onClick={() => { setEditTarget(v); setError(""); }}
                        className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white" : "border-gray-200 text-gray-500 hover:bg-slate-50 hover:text-slate-700"}`}>수정</button>
