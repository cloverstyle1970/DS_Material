        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          {pendingItems.length > 0 && (
            <div className="space-y-1.5" ref={linkRef}>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                신청건 연결
                <span className="ml-1.5 text-xs font-normal text-gray-400 dark:text-gray-500">(선택사항)</span>
              </label>
              <div className="relative">
                <button
                  type="button"
                  onClick={() => setLinkOpen(o => !o)}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 text-sm text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-slate-400 bg-white dark:bg-gray-700"
                >
                  {linkedKey ? (() => {
                    const p = pendingItems.find(x => x.key === linkedKey);
                    const isTk = p && !p.materialId.startsWith("D");
                    return p ? (
                      <span className={isTk ? "text-blue-600 dark:text-blue-400 font-medium" : "text-slate-700 dark:text-slate-300 font-medium"}>
                        #{p.reqId} [{p.siteName ?? "현장미지정"}] {p.materialName} × {p.qty} — {p.requesterName}
                      </span>
                    ) : null;
                  })() : (
                    <span className="text-gray-400 dark:text-gray-500">연결 안 함 (직접 입력)</span>
                  )}
                  <span className="text-gray-400 dark:text-gray-500 ml-2 shrink-0">{linkOpen ? "▲" : "▼"}</span>
                </button>

                {linkOpen && (
                  <ul className="absolute z-20 top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl shadow-lg overflow-y-auto max-h-56">
                    <li>
                      <button type="button"
                        onClick={() => { handleRequestLink(""); setLinkOpen(false); }}
                        className="w-full text-left px-4 py-2.5 text-sm text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        연결 안 함 (직접 입력)
                      </button>
                    </li>
                    {pendingItems.map(p => {
                      ...
                      return (
                        <li key={p.key}>
                          <button type="button"
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${p.key === linkedKey ? "bg-slate-50 dark:bg-slate-700" : ""}`}>
                            <div className={`font-medium ${isTk ? "text-blue-600 dark:text-blue-400" : "text-slate-700 dark:text-slate-300"}`}>
                              <span className={`inline-block text-[10px] px-1.5 py-0.5 rounded mr-1.5 font-semibold ${isTk ? "bg-blue-50 dark:bg-blue-900/40 text-blue-500 dark:text-blue-400" : "bg-red-50 dark:bg-red-900/40 text-red-500 dark:text-red-400"}`}>
                                {isTk ? "TK" : "DS"}
                              </span>
          ...
          <div className="space-y-1.5" ref={searchRef}>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
              ...
              {selectedItems.length > 0 && (
                <span className="ml-2 text-xs font-normal text-slate-500 dark:text-slate-400">{selectedItems.length}개 선택됨</span>
              )}
            </label>
            <div className="relative">
              <input
                ...
                placeholder={matType === "DS" ? "DS 자재 검색 (코드, 부품명, 규격)" : matType === "TK" ? "TK 자재 검색 (코드, 부품명, 규격)" : "부품명, 코드, 규격, 별칭 검색 후 클릭으로 추가"}
              ...
              {showDropdown && matResults.length > 0 && (
                <ul className="absolute z-10 top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl shadow-lg overflow-y-auto max-h-52">
                  {matResults.map(m => {
                    const checked = isSelected(m.id);
                    return (
                      <li key={m.id}>
                        <button type="button" onClick={() => toggleMaterial(m)}
                          className={`w-full text-left px-4 py-2.5 transition-colors flex items-start gap-3 ${checked ? "bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600" : "hover:bg-gray-50 dark:hover:bg-gray-700"}`}>
                          ...
                          <span className="flex-1 min-w-0">
                            <span className="flex items-center gap-1.5">
                              <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold shrink-0 ${m.id.startsWith("D") ? "bg-red-50 dark:bg-red-900/40 text-red-500 dark:text-red-400" : "bg-blue-50 dark:bg-blue-900/40 text-blue-500 dark:text-blue-400"}`}>
                            ...
                            {m.alias && <span className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 block">{m.alias}</span>}
                          </span>
                        </button>
                      </li>
                    );
          ...
          {selectedItems.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">선택된 자재</p>
              <ul className="space-y-2">
                {selectedItems.map(item => (
                  <li key={item.material.id} className="bg-slate-50 dark:bg-slate-700/50 rounded-xl px-3 py-2.5 space-y-2">
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold shrink-0 ${item.material.id.startsWith("D") ? "bg-red-50 dark:bg-red-900/40 text-red-500 dark:text-red-400" : "bg-blue-50 dark:bg-blue-900/40 text-blue-500 dark:text-blue-400"}`}>
                      ...
                      <button type="button" onClick={() => removeItem(item.material.id)}
                        className="text-gray-300 dark:text-gray-600 hover:text-red-400 transition-colors text-xl leading-none shrink-0">×</button>
                    </div>
                    <div className="flex items-center gap-2">
                      {/* 수량 */}
                      <div className="flex items-center gap-1">
                        <span className="text-xs text-gray-500 dark:text-gray-400 shrink-0">수량</span>
                        <button type="button" onClick={() => updateItem(item.material.id, { qty: Math.max(1, item.qty - 1) })}
                          disabled={item.qty <= 1}
                          className="w-6 h-6 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 disabled:opacity-30 flex items-center justify-center text-sm">−</button>
                        <input type="number" min={1} value={item.qty}
                          onChange={e => updateItem(item.material.id, { qty: Math.max(1, Number(e.target.value)) })}
                          className="w-12 text-center text-sm font-semibold text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-600 rounded-lg py-0.5 bg-white dark:bg-gray-700 focus:outline-none focus:ring-1 focus:ring-slate-400" />
                        <button type="button" onClick={() => updateItem(item.material.id, { qty: item.qty + 1 })}
                          className="w-6 h-6 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 flex items-center justify-center text-sm">+</button>
                      </div>
                      {/* 단가 */}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">호기</label>
              <select value={elevatorId} onChange={e => setElevatorId(e.target.value ? Number(e.target.value) : "")}
                disabled={elevators.length === 0}
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 text-sm text-gray-800 dark:text-gray-100 bg-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-slate-400 disabled:bg-gray-50 dark:disabled:bg-gray-700/50 disabled:text-gray-400">
          ...
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">취소</button>
