  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 409) {
        setError(`이미 등록된 수리품 코드입니다 (${repairId})`);
      } else {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">원본: <span className="font-mono">{parent.id}</span> → 수리품: <span className="font-mono font-semibold text-purple-600 dark:text-purple-400">{repairId}</span></p>
          </div>
        ...
        <div className="mx-6 mt-4 px-4 py-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl text-xs text-gray-600 dark:text-gray-400 space-y-1.5">
          <div className="flex gap-3">
            <span className="text-gray-400 dark:text-gray-500 shrink-0 w-16">원본명</span>
            <span className="font-medium text-gray-800 dark:text-gray-200">{parent.name}</span>
          </div>
          <div className="flex gap-3">
            <span className="text-gray-400 dark:text-gray-500 shrink-0 w-16">수리품 코드</span>
            <span className="font-mono font-bold text-purple-600 dark:text-purple-400">{repairId}</span>
            <span className="bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 px-1.5 py-0.5 rounded text-[10px] font-semibold">수리품</span>
          </div>
