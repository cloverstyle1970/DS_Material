    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 px-5 py-4">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-sm font-medium text-gray-700 shrink-0">조회 조건</span>
          {/* 기간 단축 버튼 */}
          <div className="flex rounded-lg overflow-hidden border border-gray-200">
            {[
              ...
              <button key={label} type="button" onClick={fn}
                className="px-3 py-2 text-xs text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition-colors border-r border-gray-200 last:border-0">
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
              className="px-2.5 py-2 rounded-lg border border-gray-200 text-xs text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-400" />
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">조회 입고 수량</p>
          <p className="text-2xl font-bold text-blue-600">{totalIn.toLocaleString()}<span className="text-sm font-normal text-gray-400 ml-1">건</span></p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">조회 출고 수량</p>
          <p className="text-2xl font-bold text-orange-500">{totalOut.toLocaleString()}<span className="text-sm font-normal text-gray-400 ml-1">건</span></p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">관련 현장 수</p>
          <p className="text-2xl font-bold text-slate-700">{siteStats.length}<span className="text-sm font-normal text-gray-400 ml-1">곳</span></p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">투입 자재 종수</p>
          <p className="text-2xl font-bold text-purple-600">{materialStats.length}<span className="text-sm font-normal text-gray-400 ml-1">종</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-gray-100">
            <h3 className="text-sm font-semibold text-gray-700">현장별 투입 현황</h3>
          </div>
          {loading ? (
            <div className="py-12 text-center text-gray-400 text-sm">데이터 로딩 중...</div>
          ) : siteStats.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-50">
                {siteStats.map(s => (
                  <tr key={s.site} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium text-gray-800 max-w-[160px] truncate">{s.site}</td>
                    ...
                    <td className="px-4 py-3 text-gray-500 tabular-nums">{s.materials.size}</td>
                    <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{fmtDate(s.lastDate)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-gray-100">
            <h3 className="text-sm font-semibold text-gray-700">
              자재별 투입 현황
              {selectedSite && <span className="ml-2 text-xs text-gray-400 font-normal">({selectedSite})</span>}
            </h3>
          </div>
          {loading ? (
            <div className="py-12 text-center text-gray-400 text-sm">데이터 로딩 중...</div>
                ...
                <tbody className="divide-y divide-gray-50">
                  {materialStats.map(m => (
                    <tr key={m.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium text-gray-800 max-w-[150px] truncate">{m.name}</td>
                      <td className="px-4 py-3 font-mono text-xs text-slate-500 whitespace-nowrap">{m.id}</td>
                      ...
                      <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{fmtDate(m.lastDate)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {filtered.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-700">입출고 이력</h3>
                  ...
                  <tr key={`${t.type}-${t.id}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        t.type === "입고" ? "bg-blue-50 text-blue-600" : "bg-orange-50 text-orange-600"
                      }`}>{t.type}</span>
                    </td>
                    <td className="px-4 py-3 font-medium text-gray-800 max-w-[180px] truncate">{t.materialName}</td>
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{t.userName}</td>
                  </tr>
                ))}
              </tbody>
            </table>
