  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center gap-4 flex-wrap">
        <span className="text-sm font-medium text-gray-700 shrink-0">조회 기준</span>
        {/* 조회 단위 */}
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button onClick={() => setShowChart(v => !v)}
            className="px-3.5 py-2 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 transition-colors">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">총 입고 수량</p>
          <p className="text-2xl font-bold text-blue-600">{totalInQty.toLocaleString()}<span className="text-sm font-normal text-gray-400 ml-1">건</span></p>
          {totalInAmt > 0 && <p className="text-xs text-gray-400 mt-1">{totalInAmt.toLocaleString()}원</p>}
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">총 출고 수량</p>
          <p className="text-2xl font-bold text-orange-500">{totalOutQty.toLocaleString()}<span className="text-sm font-normal text-gray-400 ml-1">건</span></p>
          {totalOutAmt > 0 && <p className="text-xs text-gray-400 mt-1">{totalOutAmt.toLocaleString()}원</p>}
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">순 재고 증감</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1">처리 자재 종수</p>
          <p className="text-2xl font-bold text-slate-700">{pivotData.length.toLocaleString()}<span className="text-sm font-normal text-gray-400 ml-1">종</span></p>
        </div>
      </div>
      ...
      {showChart && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-gray-700">입고 vs 출고 수량 비교</h3>
          ...
          {loading ? (
            <div className="h-40 flex items-center justify-center text-gray-400 text-sm">데이터 로딩 중...</div>
          ) : filteredTx.length === 0 ? (
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-700">자재별 입출고 상세</h3>
          <span className="text-xs text-gray-400">{pivotData.length}종</span>
        </div>

        {loading ? (
          <div className="py-16 text-center text-gray-400">데이터 로딩 중...</div>
        ) : pivotData.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-gray-500 whitespace-nowrap sticky left-0 bg-gray-50 z-10 min-w-[180px]">자재명</th>
                  <th className="px-3 py-3 text-left font-medium text-gray-400 whitespace-nowrap font-mono min-w-[120px]">코드</th>
                  {periods.map(p => (
                    <th key={p} className="px-3 py-3 text-center font-medium text-gray-500 whitespace-nowrap min-w-[80px]">
                      {periodDisplay(p, viewMode)}
                    </th>
                  ))}
                  <th className="px-4 py-3 text-center font-semibold text-gray-600 whitespace-nowrap bg-gray-100 min-w-[80px] sticky right-0">합계</th>
                </tr>
              </thead>
                ...
                {pivotData.map(m => {
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 font-medium text-gray-800 whitespace-nowrap sticky left-0 bg-white">{m.name}</td>
                      <td className="px-3 py-3 font-mono text-gray-400 whitespace-nowrap">{m.id}</td>
                      {periods.map(p => {
                      ...
                      <td className="px-4 py-3 text-center font-bold text-gray-700 bg-gray-50 sticky right-0">{rowTotal}</td>
                    </tr>
                  );
                })}
                {/* 합계 행 */}
                <tr className="bg-gray-100 font-semibold border-t-2 border-gray-200">
                  <td className="px-4 py-3 text-gray-700 sticky left-0 bg-gray-100">합계</td>
                  <td className="px-3 py-3" />
                  {periods.map(p => {
                    const total = pivotData.reduce((s, m) => s + (m.periods[p]?.in ?? 0) + (m.periods[p]?.out ?? 0), 0);
                    return (
                      <td key={p} className="px-3 py-3 text-center text-gray-700">
                        {total > 0 ? total : <span className="text-gray-300">—</span>}
                      </td>
                    );
                  })}
                  <td className="px-4 py-3 text-center text-gray-800 bg-gray-200 sticky right-0">
