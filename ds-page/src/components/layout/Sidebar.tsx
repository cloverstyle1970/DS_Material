      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-3">
          ...
          {NAV_GROUPS.map((group, gi) => {
            ...
            return (
              <div key={group.id} className={`px-2 ${gi < NAV_GROUPS.length - 1 ? "mb-5" : ""}`}>
                {/* 그룹 헤더 */}
                <div className="px-3 mb-1.5 flex items-center gap-2">
                  <span className={`text-[10px] font-semibold uppercase tracking-widest whitespace-nowrap ${group.color}`}>
                    {group.label}
                  </span>
        ...
        <div className="px-4 py-4 border-t border-white/5 shrink-0">
          ...
          <button type="button" onClick={handleLogout}
            className="w-full text-center text-xs text-slate-500 hover:text-slate-300 py-1.5 rounded-lg hover:bg-white/5 transition-colors">
            로그아웃
          </button>
        </div>
      </aside>
