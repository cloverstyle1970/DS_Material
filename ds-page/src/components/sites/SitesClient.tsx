...
import { useAuth, isViewOnly, canManageSites } from "@/context/AuthContext";
...
function companyBadge(type: string | null) {
  const style = (type && COMPANY_STYLES[type]) ?? "bg-gray-100 text-gray-500 border border-gray-200";
  return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${style}`}>{type ?? "기타"}</span>;
}

// ── 호기 등록/수정 모달 ───────────────────────────────────────
const fieldCls = "w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-base font-semibold text-gray-800">{isEdit ? "호기 수정" : "호기 등록"}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            ...
            <input value={siteName} readOnly className={fieldCls + " bg-gray-50 text-gray-500"} />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">호기명 <span className="text-red-500">*</span></label>
            <input value={unitName} onChange={e => setUnitName(e.target.value)} placeholder="예: 1호기" required className={fieldCls} />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">승강기 번호</label>
...
function AddSiteModal({ onClose, onSaved, editSite }: AddSiteModalProps) {
  ...
  const [fax,              setFax]               = useState(editSite?.fax ?? "");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b shrink-0">
          <h2 className="text-base font-semibold text-gray-800">{isEdit ? "현장 수정" : "현장 등록"}</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-600 text-lg">×</button>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-3 overflow-y-auto flex-1">
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-600 mb-1">현장명 <span className="text-red-500">*</span></label>
              <input value={name} onChange={e => setName(e.target.value)} required className={fieldCls} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">회사구분</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-gray-600">비상통화장치</label>
              <button type="button"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">담당자 HP</label>
              <input value={managerPhone} onChange={e => setManagerPhone(e.target.value)} className={fieldCls} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">담당자 메일</label>
              <input type="email" value={managerEmail} onChange={e => setManagerEmail(e.target.value)} className={fieldCls} />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">소재지 (도로명주소)</label>
            <div className="flex gap-2">
              <input value={address} onChange={e => setAddress(e.target.value)}
                placeholder="도로명주소 검색 또는 직접 입력" className={`${fieldCls} flex-1`} />
              <button type="button" onClick={searchAddress}
                className="shrink-0 px-3 py-2 rounded-lg border border-gray-200 text-xs text-gray-600 hover:bg-gray-50 whitespace-nowrap">
                🔍 주소 검색
              </button>
            </div>
            {postalCode && <p className="mt-1 text-xs text-gray-400">우편번호: {postalCode}</p>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">출입정보</label>
              <input value={entryInfo} onChange={e => setEntryInfo(e.target.value)} placeholder="도어락 번호 등" className={fieldCls} />
            </div>
          </div>
          {/* 하자 정보 */}
          <div className="border-t pt-3">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2.5">하자 정보</p>
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">하자기 대수</label>
                  <input type="number" min={0} value={warrantyCount} onChange={e => setWarrantyCount(e.target.value)} placeholder="0" className={fieldCls} />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-gray-600 mb-1">하자기 호기정보</label>
                  <input value={warrantyUnits} onChange={e => setWarrantyUnits(e.target.value)} placeholder="예: 1호기, 3호기" className={fieldCls} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">하자기간 시작</label>
                  <input type="date" value={warrantyStart} onChange={e => setWarrantyStart(e.target.value)} className={fieldCls} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">하자기간 종료</label>
          ...
          <div>
            ...
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none" />
          </div>
          {error && <p className="text-sm text-red-500 bg-red-50 rounded-lg px-4 py-2.5">{error}</p>}
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-lg border border-gray-200 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50">취소</button>
...
export default function SitesClient({ initial, elevators }: Props) {
  ...
  const { user } = useAuth();
  const canEdit  = user ? canManageSites(user) : false;
  ...
  const displaySites = useMemo(() => {
    if (!q) return sites.slice(0, INITIAL_SHOW);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* ── 좌측: 현장 목록 ─────────────────────────────────── */}
        <div className="w-80 xl:w-96 shrink-0 flex flex-col border-r border-gray-200 bg-white">
          {/* 검색 */}
          <div className="shrink-0 p-3 bg-gray-50 border-b border-gray-200">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder:text-gray-400 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
              {query && (
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          {companyBadge(site.companyType)}
                          {site.primaryInspector && (
                            <span className="text-xs text-gray-500">{site.primaryInspector}</span>
                          )}
                        </div>
                      </div>
                      <div className="shrink-0 text-right">
                        {unitCount > 0 ? (
                          ...
                          <span className="text-xs text-gray-300">호기 없음</span>
                        )}
                      </div>
                    </div>
                  </button>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50">
          {!selected ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 min-h-64">
              <p className="text-5xl mb-4">🏗️</p>
              <p className="text-base font-medium text-gray-500">현장을 선택하세요</p>
            ...
            <div className="p-6 space-y-5">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      {companyBadge(selected.companyType)}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">계약 정보</h3>
                  <InfoRow label="계약구분"   value={selected.contractType} />
                  <InfoRow label="계약일자"   value={selected.contractDate} />
                  <InfoRow label="계약기간"   value={
                    selected.contractStart || selected.contractEnd
                      ? `${selected.contractStart ?? ""} ~ ${selected.contractEnd ?? ""}`
                      : null
                  } />
                  <InfoRow label="하자종료"   value={selected.warrantyEnd} />
                  <InfoRow label="출입정보"   value={selected.entryInfo} />
                  <InfoRow label="비고"       value={selected.note} />
                  <InfoRow label="거래처"     value={selected.vendor} />
                </div>

                {/* 담당자 정보 */}
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">담당자 정보</h3>
                  <InfoRow label="주점검자"    value={selected.primaryInspector} />
                  <InfoRow label="보조점검자1" value={selected.subInspector} />
                  <InfoRow label="보조점검자2" value={selected.subInspector2} />
                  <InfoRow label="현장전화"    value={selected.sitePhone} />
                  <InfoRow label="핸드폰"     value={selected.siteMobile} />
                  <InfoRow label="담당자 HP"  value={selected.managerPhone} />
                </div>
              </div>
              ...
              {(selected.warrantyCount != null || selected.warrantyStart) && (
                <div className="bg-white rounded-xl border border-amber-200 p-4">
                  <h3 className="text-xs font-semibold text-amber-600 uppercase tracking-wide mb-3">⚠ 하자 정보</h3>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    {selected.warrantyCount != null && (
                      <div>
                        <span className="text-xs text-gray-400 block mb-0.5">하자기 대수</span>
                        <span className="font-medium text-gray-800">{selected.warrantyCount}대</span>
                      </div>
                    )}
                    {selected.warrantyUnits && (
                      <div>
                        <span className="text-xs text-gray-400 block mb-0.5">하자기 호기</span>
                        <span className="font-medium text-gray-800">{selected.warrantyUnits}</span>
                      </div>
                    )}
                    {(selected.warrantyStart || selected.warrantyEnd) && (
                      <div className="col-span-2">
                        <span className="text-xs text-gray-400 block mb-0.5">하자기간</span>
              ...
              {/* 비상통화장치 */}
              {selected.emergencyDevices && selected.emergencyDevices.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">비상통화장치</h3>
                  <div className="space-y-1.5">
                    {selected.emergencyDevices.map((d, i) => (
                      <div key={i} className="flex items-center gap-3 text-sm">
                        <span className="font-mono font-medium text-gray-800">{d.number}</span>
                        {d.note && <span className="text-gray-400 text-xs">— {d.note}</span>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-gray-700">호기 목록</h3>
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b border-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">호기명</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">승강기 번호</th>
                          {canEdit && !viewOnly && (
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {selectedElevators.map((e, idx) => (
                          <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3 font-medium text-gray-800 whitespace-nowrap">{e.unitName ?? "—"}</td>
                            <td className="px-4 py-3 font-mono text-xs text-blue-600 whitespace-nowrap">{e.elevatorNo ?? "—"}</td>
                            {canEdit && !viewOnly && (
                              <td className="px-4 py-3">
                                <div className="flex gap-1">
                                  <button
                                    onClick={() => { setEditElevator(e); setShowElevatorForm(true); }}
                                    className="text-xs px-2 py-1 rounded border border-gray-200 text-gray-500 hover:bg-slate-50 hover:text-slate-700 transition-colors">
                                    수정
                                  </button>
                                  <button
              ...
              {selected.note && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                  <h3 className="text-xs font-semibold text-amber-700 mb-1.5">비고</h3>
                  <p className="text-sm text-amber-900">{selected.note}</p>
                </div>
              )}
            </div>
          )}
      ...
      {deleteElevatorTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40"
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6"
            ...
            <h3 className="text-base font-semibold text-gray-800 mb-2">호기 삭제</h3>
            <p className="text-sm text-gray-600 mb-1">
              <span className="font-medium text-gray-800">{deleteElevatorTarget.unitName ?? "—"}</span>
              {deleteElevatorTarget.elevatorNo && (
                <span className="text-gray-400"> ({deleteElevatorTarget.elevatorNo})</span>
              )}
              을(를) 삭제하시겠습니까?
            </p>
