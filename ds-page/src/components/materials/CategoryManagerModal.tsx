...
function EditableRow({
  ...
  return (
    <div className="flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-gray-50 group">
      <span className="font-mono text-xs text-gray-400 w-6 shrink-0">{item.code}</span>
      {editing ? (
        <>
          <input autoFocus value={draft} onChange={e => setDraft(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter") save(); if (e.key === "Escape") { setDraft(item.label); setEditing(false); } }}
            className="flex-1 text-sm border border-slate-300 rounded px-2 py-0.5 focus:outline-none focus:ring-1 focus:ring-slate-400" />
        ...
        <>
          ...
          <button type="button" onClick={() => setEditing(true)} className="text-xs text-slate-400 hover:text-slate-700 opacity-0 group-hover:opacity-100 transition-opacity">수정</button>
  ...
  return (
    <div className="flex items-center gap-2 mt-2 shrink-0">
      <input value={label} onChange={e => setLabel(e.target.value)} placeholder="새 항목 이름"
        ...
        className="flex-1 text-sm text-gray-800 border border-dashed border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-slate-400 placeholder-gray-300" />
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <h2 className="text-base font-semibold text-gray-800">분류 관리</h2>
        ...
        <div className="flex flex-1 min-h-0 divide-x divide-gray-100 overflow-hidden">
          ...
          <div className="w-1/3 flex flex-col min-h-0 p-4">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 shrink-0">대분류</p>
            <div className="flex-1 overflow-y-auto space-y-0.5 min-h-0">
              {store.major.map(m => (
                <div key={m.code}
                  className={`cursor-pointer rounded-lg transition-colors ${selMajor === m.code ? "bg-slate-100" : ""}`}
          ...
          <div className="w-1/3 flex flex-col min-h-0 p-4">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 shrink-0">
              중분류 {selMajor ? <span className="text-slate-400 font-normal normal-case">({store.major.find(m => m.code === selMajor)?.label})</span> : ""}
            </p>
            {!selMajor ? (
              <p className="text-sm text-gray-300 mt-4 text-center">대분류를 선택하세요</p>
            ) : (
              <>
                <div className="flex-1 overflow-y-auto space-y-0.5 min-h-0">
                  {midList.map(m => (
                    <div key={m.code}
                      className={`cursor-pointer rounded-lg transition-colors ${selMid === m.code ? "bg-slate-100" : ""}`}
          ...
          <div className="w-1/3 flex flex-col min-h-0 p-4">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 shrink-0">
              소분류 {selMid ? <span className="text-slate-400 font-normal normal-case">({midList.find(m => m.code === selMid)?.label})</span> : ""}
            </p>
            {!selMid ? (
              <p className="text-sm text-gray-300 mt-4 text-center">중분류를 선택하세요</p>
            ) : (
              <>
                <div className="flex-1 overflow-y-auto space-y-0.5 min-h-0">
                  {subList.map(s => (
        ...
        <div className="px-6 py-3 border-t border-gray-100 shrink-0 flex justify-end">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">닫기</button>
        </div>
      </div>
    </div>
  );
