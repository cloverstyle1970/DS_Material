/**
 * vendors, users, sites, elevators ?�이?��? Excel?�서 Supabase�?import.
 * ?�행: node scripts/import-all.mjs
 */
...
async function upsert(table, rows, key = "id") {
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    if (error) { console.error(`  ${table} 배치 ?�류:`, error.message); }
    else { total += batch.length; process.stdout.write(`\r  ${table}: ${total}/${rows.length}�?); }
  }
  console.log(`\r  ${table}: ${total}�??�료          `);
}

// ?�?� 1. vendors ?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�?�
console.log("\n[1/4] 거래�?import...");
{
  const wb = XLSX.readFile(path.join(ROOT, "거래처리?�트.xlsx"));
...
console.log("\n[3/4] ?�장 import...");
{
  const wb = XLSX.readFile(path.join(ROOT, "?�장리스??xlsx"));
  const rows = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, defval: "" });
  const seen = new Set();
  const sites = [];
  let id = 1;
  for (const row of rows.slice(1)) {
    ...
    sites.push({
      id: id++,
      name,
      company_type:      str(row[1]) === "?�사" ? "DS" : str(row[1]),
...
console.log("\n[4/4] ?�기 import...");
{
  const wb = XLSX.readFile(path.join(ROOT, "?�장?�기?�보.xlsx"));
  const rows = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, defval: "" });
  const elevators = [];
  let id = 1;
  for (const row of rows.slice(1)) {
