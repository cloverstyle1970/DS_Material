...
const CHUNK = 500;
const lines = [];
lines.push("-- materials ?åÏù¥Î∏?Ï¥àÍ∏∞?????¨ÏÇΩ??);
lines.push("-- ?ùÏÑ±: " + new Date().toISOString());
lines.push("-- Ï¥?" + partList.length + "Í±?);
lines.push("");
lines.push("BEGIN;");
...
console.log(`?ëÏÑ± ?ÑÎ£å: ${out} (${partList.length}Í±? ${sizeKB} KB)`);
